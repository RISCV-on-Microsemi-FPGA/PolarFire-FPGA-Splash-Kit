...
if {"$config" == "CFG1"} then {
	if {[file exists $project_dir_CFG1] == 1} then {
		...
		source ./import/components/IMA_CFG1/import_component_and_constraints_pf_splash_kit_es_rv32ima_cfg1.tcl
		save_project
        base_design_built
	}
	...
	if {[file exists $project_dir_CFG2] == 1} then {
		...
		download_cores_all_cfgs
		source ./import/components/IMA_CFG2/import_component_and_constraints_pf_splash_kit_es_rv32ima_cfg2.tcl
		save_project
        base_design_built
	}
} elseif {"$config" != ""} then {
