...
set Libero_project_name_CFG1 PF_Splash_Kit_ES_MiV_RV32IMC_CFG1_BaseDesign

set project_folder_name_CFG2 MiV_CFG2_BD
set project_dir_CFG2 "./$project_folder_name_CFG2"
set Libero_project_name_CFG2 PF_Splash_Kit_ES_MiV_RV32IMC_CFG2_BaseDesign

set project_folder_name_CFG3 MiV_CFG3_BD
set project_dir_CFG3 "./$project_folder_name_CFG3"
set Libero_project_name_CFG3 PF_Splash_Kit_ES_MiV_RV32IMC_CFG3_BaseDesign
...
if {"$config" == "CFG1"} then {
	...
	if {[file exists $project_dir_CFG3] == 1} then {
		project_exists $Libero_project_name_CFG3
	} else {
		create_new_project_label $Libero_project_name_CFG3
		new_project -location $project_dir_CFG3 -name $Libero_project_name_CFG3 -project_description {} -block_mode 0 -standalone_peripheral_initialization 0 -instantiate_in_smartdesign 1 -ondemand_build_dh 1 -hdl {VERILOG} -family {PolarFire} -die {MPF300T_ES} -package {FCG484} -speed {-1} -die_voltage {1.0} -part_range {IND} -adv_options {IO_DEFT_STD:LVCMOS 1.8V} -adv_options {RESTRICTPROBEPINS:1} -adv_options {RESTRICTSPIPINS:0} -adv_options {TEMPR:IND} -adv_options {UNUSED_MSS_IO_RESISTOR_PULL:None} -adv_options {VCCI_1.2_VOLTR:EXT} -adv_options {VCCI_1.5_VOLTR:EXT} -adv_options {VCCI_1.8_VOLTR:EXT} -adv_options {VCCI_2.5_VOLTR:EXT} -adv_options {VCCI_3.3_VOLTR:EXT} -adv_options {VOLTR:IND} 
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$config" == "CFG1"} then {
		export_prog_job \
			-job_file_name {PF_Splash_Kit_ES_MiV_RV32IMC_CFG1_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {PF_Splash_Kit_ES_MiV_RV32IMC_CFG2_BaseDesign} \
		...
		save_project
			...
			export_prog_job \
			-job_file_name {PF_Splash_Kit_ES_MiV_RV32IMC_CFG3_BaseDesign} \
